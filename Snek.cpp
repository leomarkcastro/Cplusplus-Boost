...
///LAHAT NG NASA TAAS CPPBOOST

///dito to start ng code
...
int main(){
    ...
    int life = 3;
    float score = 0;
    int eatsize = 5; //how much will he grow after eating food
    ...
    setbuffersize("max", resolutionX, resolutionY);
    ...
    boxfill((B_BLUE), 50, 1, 1, 2);
    boxfill((B_WHITE), 50, 30, 1, 3);
    gotoxy(4,3); setcolor(B_WHITE|F_BLACK); printf("LIFE: %d", life);
    gotoxy(36,3); setcolor(B_WHITE|F_BLACK); printf("SCORE: %.0f", score);
    gotoxy(2,1); setcolor(B_BLUE|F_WHITE); say " COMMAND PROMPT SNEK ";
    gotoxy(48,1); setcolor(B_RED|F_WHITE); say " X ";
    gotoxy(38,1); setcolor(B_RED|F_WHITE); say " RESET ";
    boxfill((255), 46, 28, 3, 4, ' ');
    ...
    do{
    FoodX = 3 + (rand() % 35);
    FoodY = 4 + (rand() % 25);} while(FoodX == snek[0].x && FoodY == snek[0].y);

    int bod = 1;


    boxfill((255), 46, 28, 3, 4, ' ');
    gotoxy(snek[0].x,snek[0].y); setcolor(F_RED); say char(BLOCK);
    tick = clock();
    ...
    while(game){
        ...
        if(kbhit()){
            ...
            switch(inp){
                case 'a':if(moveX == 0){moveY = 0; moveX = -1; movenow = 1;} break;
                case 'd':if(moveX == 0){moveY = 0; moveX = 1; movenow = 1;} break;
                case 's':if(moveY == 0){moveX = 0; moveY = 1; movenow = 1;} break;
                case 'w':if(moveY == 0){moveX = 0; moveY = -1; movenow = 1;} break;
                case ' ':getch(); break;
            }
        ...
        if(tack - tick > 50 || movenow == 1){
            ...
            if(snek[0].x == FoodX && snek[0].y == FoodY){
                ...
                do{
                FoodX = 3 + (rand() % 35);
                FoodY = 4 + (rand() % 25);} while(getcursorchar(FoodX, FoodY) != ' ');

                for(int i = 0; i < eatsize; i++){
                snek[preexist].exist = TRUE;
                preexist++;}
            }

            ///*
            if(moveX == -1){if (getcursorchar(snek[0].x - 1, snek[0].y) == 'Z') {game = 0; goto enda;} }
            else if(moveX == 1){if (getcursorchar(snek[0].x + 1, snek[0].y) == 'Z') {game = 0; goto enda;} }
            else if(moveY == -1){if (getcursorchar(snek[0].x, snek[0].y - 1) == 'Z') {game = 0; goto enda;} }
            else if(moveY == 1){if (getcursorchar(snek[0].x, snek[0].y + 1) == 'Z') {game = 0; goto enda;} }
                //*/
            ...
            snek[0].x += moveX; snek[0].y += moveY;

            if(snek[0].x < 3 || snek[0].x > 48 || snek[0].y < 4 || snek[0].y > 31){ game = 0; goto enda;}

            for(int i = 0; i < 1000; i++){
                if(snek[i].exist == 1){
                    gotoxy(snek[i].x,snek[i].y); setcolor(F_RED | B_RED); say 'Z';}}

            score += 0.1;
            gotoxy(FoodX, FoodY); setcolor(B_GREEN | F_GREEN); say 'A';
            gotoxy(36,3); setcolor(B_WHITE|F_BLACK); printf("SCORE: %.0f", score);
            enda:
    ...
    for(int i = 0; i < preexist; i++){
        ...
        for(int j = 0; j < preexist; j++){
            if(snek[j].exist == 1)
            {gotoxy(snek[j].x,snek[j].y); setcolor(F_GREEN | B_GREEN); say 'Z';}
            //else {snek[j].x = snek[j].y = 0; }
            }
    ...
    life -= 1;
    if(life > 0){goto starta;}



    setcolor(ret);
    system("cls");
    setcolor(B_BLACK | F_WHITE); say "GAME OVER!";
    Sleep(3000);
